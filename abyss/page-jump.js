    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        menuNavItems.forEach(item => {
            item.addEventListener('click', function (e) {
                ...
                console.log("嘗試跳轉至:", targetUrl); // 除錯用

                if (targetUrl && typeof navigateTo === 'function') {
                    navigateTo(targetUrl); // 呼叫 page-jump.js 中的動畫跳轉函數
                } else if (targetUrl) {
                    window.location.href = targetUrl; // 如果動畫函數失效，執行強制跳轉
                }
            });
        });
    });
